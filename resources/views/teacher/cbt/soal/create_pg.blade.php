...
@section('content')
    ...
    <div class="level">
        <div class="level-left">
            <div class="field is-grouped">
                <div class="control">
                    <a href="{{ route('teacher.cbt.soal.index', $cbt) }}" class="button is-primary is-outlined">
                        <span class="icon">
                ...
                <div class="control">
                    <a href="{{ route('teacher.cbt.soal.show', [$cbt, $soal]) }}" class="button is-primary is-outlined">
                        <span class="icon">
    ...
    @card(['title' => 'Buat Soal (' . config('school.cbt.jenis_pertanyaan')[$soal->jenis] . ')'])
        <form action="{{ route('teacher.cbt.soal.update', [$cbt, $soal]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="field">
                <label class="label">Opsi Jawaban {{ $key }}</label>
                <div class="control">
                    <textarea id="opsi_{{ strtolower($key) }}" name="opsi[{{ $key }}]" class="textarea" placeholder="Konten Jawaban {{ strtoupper($key) }}">{{ old('opsi.' . $key, $value) }}</textarea>
                </div>
                
                {{-- UPLOAD FILE --}}
                <div style="height: 15px;"></div>
                <question-media ujian="{{ $cbt->id }}" soal="{{ $soal->id }}" field="opsi_{{ $key }}"></question-media>
                @if(! $loop->last)
                <hr>
                @endif
            </div>
            ...
            <div class="field">
                ...
                <div class="control">
                    <div class="select{{ $errors->has('jawaban') ? ' is-danger' : '' }}">
                        <select name="jawaban">
                            <option value="none">Pilih Jawaban</option>
                            @foreach(array_keys($soal->opsi) as $item)
                                <option value="{{ $item }}" {{ old('jawaban', $soal->jawaban) === $item ? 'selected' : '' }}>{{ $item }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                @if($errors->has('jawaban'))
                <p class="help is-danger">{{ $errors->first('jawaban') }}</p>
                @endif
            </div>
...
@php
    $rich_inputs = ['pertanyaan'];
    foreach (config('school.cbt.opsi_jawaban.' . $cbt->kelas->sekolah->jenjang) as $alfabet) {
        $rich_inputs[] = 'opsi_' . strtolower($alfabet);
    }
@endphp
@section('extra_script')
    ...
    <script>
        ...
        // var inputs = ['pertanyaan', 'opsi_a', 'opsi_b', 'opsi_c', 'opsi_d'];
        var inputs = @json($rich_inputs);
